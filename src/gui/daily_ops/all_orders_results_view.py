...
# ── Column spec ────────────────────────────────────────────────────────────────
# Same layout as the daily ops results but without the "Type / Zone" notes column.
# The "notes" column is repurposed to show order value.

_ALL_ORDERS_COL_SPEC = {
    "#0":          ("Order No.",   120),
    "date":        ("Date",         70),
...
class AllOrdersResultsView(ctk.CTkFrame):
    """
    Results & Dispatch view for Show All Orders.

    Shows all fetched orders in a single flat list (no tabs).
    Supports assignment filtering, order detail/freight booking, bulk actions,
    and per-order refresh. No session persistence, no pick list export.
    """
    ...
    def __init__(self, master, window, on_back, **kwargs):
        ...
        self._neto_orders: list = list(window.all_orders_neto_orders)
        self._ebay_orders: list = list(window.all_orders_ebay_orders)

        # (platform, order_id) tuples for orders hidden from the list
    ...
    def _build_list_page(self, parent):
        ...
        self._tree = OrderTreeview(
            list_container,
            col_spec=_ALL_ORDERS_COL_SPEC,
            table_id="daily_all_orders_results",
        ...
        # ── Action bar (hidden until ≥1 order selected) ──────────────────
    ...
    def show(self):
        ...
        if self._loaded:
            return
        self._loaded = True
        self._neto_orders = list(self._window.all_orders_neto_orders)
        self._ebay_orders = list(self._window.all_orders_ebay_orders)
