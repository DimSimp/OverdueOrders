...
def export_picking_list_xlsx(items: list[dict], output_path: str) -> None:
    ...
    Columns: SKU | Description | QTY | ✓ (Picked)
    ...
    alternate white/grey backgrounds, bold centred QTY, and an empty Picked
    checkbox column.
    ...
    desc_width = min(max_desc_len + 2, _A4_USABLE - SKU_WIDTH - QTY_WIDTH - CHECKBOX_WIDTH)
    ...
    ws.column_dimensions["D"].width = CHECKBOX_WIDTH
    ...
    # Alternating row fill (every second data row gets a light grey background)
    ...
    for col, text in enumerate(["SKU", "Description", "QTY", "✓"], start=1):
        ...
        cell.alignment = Alignment(horizontal="center" if col in (3, 4) else "left")
    ...
    for item in items:
        ...
        # Alternating row fill: odd rows (0-indexed) get grey
        row_fill = grey_fill if data_row_count % 2 == 1 else None
        data_row_count += 1
        ...
        for col, value in enumerate([item["sku"], item["description"], item["qty"], ""], start=1):
            ...
            if col == 3:  # QTY — centered and bold
                cell.font = Font(bold=True, size=11)
