...
- Daily Sales modal now has calendar-backed date range filtering (default today), transaction-number/receipt-barcode search, refund handoff to Till, and net summary totals that subtract refunds.
- Till linked refund loading now preserves the original transaction number until completion or **Clear Cart**, restores original payment method/amount split, and is shared by lookup, Daily Sales, and Customer Sale History.
- Quote sale type now saves draft quote transactions for linked customers, writes transaction lines without payment or stock movement, assigns a `quote_number`, and opens an A4 quote document preview with print/save/email/close actions.
...
- [~] Customer management UI (Plan 05) — Customers tab is live in the POS window with search, pagination, customer detail panel, sale history, quote history, CSV import, create/edit modal, invoice/shipping addresses, profile discount selection, and right-click `Load in Till`. Sale History supports newest-first 100-row paging plus calendar-backed date range filters and can start linked refunds in the Till. Quotes tab lists Till-saved draft quotes and can preview the A4 quote PDF. Invoices, Repairs, Deposits, and Audit tabs are present as placeholders and still need full implementation.
...
*Last updated: 2026-04-27 — Quote sale type and Customer Quotes tab documented; Daily Sales/refund and Customer Sale History updates remain documented.*
