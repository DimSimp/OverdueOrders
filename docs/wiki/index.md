...
- [~] Customer management UI (Plan 05) — Customers tab is live in the POS window with search, pagination, customer detail panel, sale history, CSV import, create/edit modal, invoice/shipping addresses, profile discount selection, and right-click `Load in Till`. Sale History supports newest-first 100-row paging plus calendar-backed date range filters and can start linked refunds in the Till. Quotes, Invoices, Repairs, Deposits, and Audit tabs are present as placeholders and still need full implementation.
...
*Last updated: 2026-04-27 — Daily Sales date range/search/refund updates documented; Customer Sale History date filtering and Audit placeholder documented; Till linked refunds now preserve source transaction number and original payment split.*
